...
\end{figure}
    ...
    As shown in Figure \ref{error1}, the SVR model generally exhibited better prediction results than other individual models, while the LR model is the worst showing substantial deviation from the target line (denoting relative large differences between the observed and the predicted crack widths). For the initial crack widths less than 60 $\mu m$ before self-healing, the variations shown in SVR model are smaller than those presented in other models. However, the CRAT model showed smaller variations for the crack widths between 60 and 100 $\mu m$ compared to the SVR and BPNN models. For crack widths larger than 100 $\mu m$, the SVR model performed the best showing the smallest variations. In terms of accuracy among the individual models, SRV performed the best, followed by CRAT, BPNN and LR. This is consistent with the MAE results indicated in Table \ref{per}.
    
    	

	
    The performance of ensemble methods using Bagging and Adaboost are shown in Figures \ref{error2} and \ref{error3}. In general, the ensemble models  bag\_CRAT and Ada\_CRAT   exhibited lower variations in self healing results compared to other ensemble models. In particular, the variations observed in CRAT model for crack widths between 60 and 100 $\mu m$ are much smaller (Figure 10e). The MAE values (i.e. between the predicted crack width and the experimental observed crack width) for bag\_CRAT and Ada\_CRAT are 4.093 and 4.207, respectively. However, the variations among BPNN, Ada\_BPNN and bag\_BPNN are not significant. Similar variations can be found when comparing SVR with Ada\_SVR and bag\_SVR. After stacking, the variations are much reduced and in the range between -20 and 20 μm as shown in Figure \ref{error4}. The MAE value for Stack\_LR is 3.934, which is the smallest among all the ML models studied.  
    
    
    It is known that smaller crack width is favourable for autogenous healing in concrete \cite{herbert2013selfhealing,liu2017influence} as small cracks consume less repair products to complete self-healing \cite{de2018review}. However, larger crack width will not heal completely or just heal partially. Therefore, higher variations in self healing results compared to the predicted results are observed. As shown in Figures \ref{error1} to \ref{error4}, the variations between the observed and predicted results increased with the increase of crack width. For the initial crack width below 20, the variations are less than XX\%, which is much lower that that observed for crack widths between 40-80 $\mu m$. For the crack width over 100 $\mu m$,the LR, bag\_LR and Ada\_LR models show much higher variations compared with other ML models. Thus, it can be concluded that ensemble methods can significantly improve the prediction accuracy of individual methods especially for smaller crack widths. Besides, the stack\_LR model that combines multiple prediction models (BPNN, CRAT, SVR and LR) is superior to all other models shown the lowest variations. 
...
%	\end{figure}
	...
	\begin{enumerate}
	   ...
	   \item  For the initial crack widths less than 60μm, the variations shown in SVR model are smaller than those presented in other models. However, the CRAT model showed smaller variations for the crack widths between 60 and 100μm compared to the SVR and BPNN models. For crack widths larger than 100μm, the SVR model performed the best showing the smallest variations. 
